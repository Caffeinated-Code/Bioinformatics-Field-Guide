# Bioinformatics Field Guide Content

This folder contains draft reader-facing content for the first 12 weeks of the blog.

Editorial principles:
- Keep the EB-1A goal in mind by creating original, useful, reusable resources.
- Write for beginners, practitioners, researchers, and industry teams without losing clarity.
- Keep articles readable in about 7 minutes.
- Cite tools, papers, datasets, authors, and institutions clearly.
- Do not include internal prompts, hidden drafting notes, or AI process language in posts.
- Review every post for scientific accuracy before publication.

## 12-Week Launch Sequence

| Week | Draft | Core asset |
|---|---|---|
| 1 | [How to Set Up Your Laptop for Bioinformatics](posts/week-01-laptop-setup-for-bioinformatics.md) | Setup checklist and reproducible environment plan |
| 2 | [What Languages Should a Bioinformatician Know?](posts/week-02-languages-for-bioinformatics.md) | Language decision tree |
| 3 | [What Biology Do You Need for Bioinformatics?](posts/week-03-biology-prerequisites.md) | Biology prerequisite map |
| 4 | [How to Think Like a Bioinformatician](posts/week-04-think-like-a-bioinformatician.md) | Bioinformatics thinking framework |
| 5 | [The File Types You Must Know](posts/week-05-bioinformatics-file-types.md) | File-format atlas |
| 6 | [The Bioinformatics Project Template](posts/week-06-reproducible-project-template.md) | GitHub-ready project template |
| 7 | [RNA-seq in 7 Minutes](posts/week-07-rnaseq-in-seven-minutes.md) | RNA-seq workflow map |
| 8 | [Differential Expression: What It Tells You and What It Does Not](posts/week-08-differential-expression.md) | Interpretation rubric |
| 9 | [PCA in Bioinformatics](posts/week-09-pca-in-bioinformatics.md) | PCA visual notebook plan |
| 10 | [Single-Cell RNA-seq vs Bulk RNA-seq](posts/week-10-single-cell-vs-bulk-rnaseq.md) | Bulk vs single-cell comparison map |
| 11 | [How to Read a Bioinformatics Paper Without Getting Fooled](posts/week-11-read-bioinformatics-paper.md) | Paper-reading scorecard |
| 12 | [Foundation Models in Bioinformatics](posts/week-12-foundation-models-bioinformatics.md) | Research-gap map |

## Pre-Publication Review Gate

Before publishing each draft:
- Confirm all factual claims and references.
- Test any commands or notebooks.
- Replace placeholders with final links.
- Add final figures or diagrams.
- Check for prompt leakage or internal notes.
- Confirm reading time.
- Decide whether the asset is free, email-gated, or paid.
